...
export default class User extends React.Component{
    ...
    userListView(){
        ...
        if(this.state.Kullanıcı<1){
            return(
                <Text style={{fontSize:15, margin:17}}>Lütfen Kullanıcı Numarası Giriniz ..</Text>
            ) 
        }
        let renderUserList=[];
                ...
                renderItem={({item})=>{
                    ...
                    return(
                       <TouchableOpacity onPress={()=>this.handlePress(item.id)} style={{minHeight: 30, backgroundColor: 'white'}}>
                           <Text style={{fontSize:18,color:'white', backgroundColor:'blue',textAlign:'center'}}>{item.id}</Text>
                       </TouchableOpacity> 
                    )
                }
                }
